...
for current_input in inputs_series:
    ...
    input_and_state_concatenated = tf.concat([current_input, current_state],1)  #(5,5) Increasing number of columns

    next_state = tf.tanh(tf.matmul(input_and_state_concatenated, W) + b) #(5,4)  Broadcasted addition
    states_series.append(next_state)
    current_state = next_state

logits_series = [tf.matmul(state, W2) + b2 for state in states_series] #Broadcasted addition (5,4)*(4,2) = 5,2
predictions_series = [tf.nn.softmax(logits) for logits in logits_series]#5,2
